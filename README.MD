...
6. `kubectl apply -f deployment/udaconnect-grpc-api.yaml` - Set up the service and deployment for the gRPC API
7. `kubectl apply -f deployment/udaconnect-kafka-api.yaml` - Set up the service and deployment for the kafka API
...
You can deploy Kafka on Kubernetes through Helm.
Instructions here:
https://docs.confluent.io/5.0.2/installation/installing_cp/cp-helm-charts/docs/index.html
...
`kubectl get pods` and `kubectl get services` - should both return `udaconnect-app`, `udaconnect-api`,`udaconnect-grpc-api`, `udaconnect-kafka-api`, and `postgres`.


These pages should also load on your web browser:
* `http://localhost:30001/` - OpenAPI Documentation (also listing the location and person creation APIs)
...
### Docker Images
Docker images are fetch from 'evandror' repository.
